"use client";

import { motion } from "framer-motion";
import Image from "next/image";
import { defaultContent } from "@/lib/content";
import { siteConfig } from "@/lib/site-config";
import type { WelcomeContent, BusinessInfo } from "@/types/content";

interface WelcomeSectionProps {
  content?: WelcomeContent;
  business?: BusinessInfo;
}

export default function WelcomeSection({ content, business }: WelcomeSectionProps) {
  const data = content || defaultContent.welcome;
  const info = business || defaultContent.business;

  return (
    <section id="ueber-mich" className="relative section-padding pb-28 bg-base-50">
      <div className="max-w-4xl mx-auto px-6 md:px-8">
        {/* Section Title */}
        <motion.h2
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.6 }}
          className="text-4xl md:text-5xl text-center mb-8"
        >
          {data.title}
        </motion.h2>

        {/* Intro Text */}
        <motion.p
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.6, delay: 0.1 }}
          className="text-text-medium text-center text-lg md:text-xl leading-relaxed mb-6 max-w-2xl mx-auto"
        >
          {data.intro}
        </motion.p>

        {/*
          Regenbogen-Badge. Die Farben sind bewusst fest und folgen NICHT der
          Palette — es ist ein etabliertes Symbol, kein Designelement.
        */}
        {siteConfig.showInclusivityBadge && (
          <motion.div
            initial={{ opacity: 0, y: 10 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.5, delay: 0.2 }}
            className="flex items-center justify-center gap-3 mb-16"
          >
            <div
              className="w-6 h-6 rounded-full flex-shrink-0"
              style={{
                background:
                  "linear-gradient(180deg, #E40303 0%, #FF8C00 20%, #FFED00 40%, #008026 60%, #24408E 80%, #732982 100%)",
              }}
            />
            <span className="text-text-medium text-sm italic">
              {data.inclusivityText}
            </span>
          </motion.div>
        )}

        {/* Portrait and Bio */}
        <div className="grid md:grid-cols-2 gap-10 md:gap-16 items-center">
          {/* Portrait */}
          <motion.div
            initial={{ opacity: 0, x: -30 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.6 }}
            className="flex justify-center"
          >
            <div className="relative">
              {/* Decorative background shape */}
              <div className="absolute -inset-4 bg-accent-100 rounded-[60%_40%_50%_50%] -rotate-6" />

              {/* Image Container */}
              <div className="relative w-64 h-80 md:w-72 md:h-96 portrait-frame overflow-hidden">
                <Image
                  src={data.image}
                  alt={info.fullName}
                  fill
                  className="object-cover object-top"
                  sizes="(max-width: 768px) 256px, 288px"
                />
              </div>
            </div>
          </motion.div>

          {/* Bio Text */}
          <motion.div
            initial={{ opacity: 0, x: 30 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.6, delay: 0.2 }}
          >
            <p className="text-text-medium text-lg leading-relaxed">
              {data.aboutText}
            </p>
          </motion.div>
        </div>

        {/* Quote - Grösser und besser lesbar */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.6, delay: 0.3 }}
          className="mt-24 text-center py-12 px-6 bg-accent-100/50 rounded-2xl"
        >
          <p className="quote-text max-w-2xl mx-auto">
            {data.quote}
          </p>
        </motion.div>
      </div>

      {/* Wave Divider */}
      <div className="absolute -bottom-px left-0 right-0">
        <svg
          viewBox="0 0 1440 80"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
          className="w-full h-auto block"
          preserveAspectRatio="none"
          style={{ marginBottom: "-1px" }}
        >
          <path
            d="M0 40C240 70 480 20 720 50C960 80 1200 30 1440 60V80H0V40Z"
            fill="var(--accent-200)"
          />
        </svg>
      </div>
    </section>
  );
}
