...
import type { NavigationItem, BusinessInfo } from "@/types/content";
...
export default function Header({ content }: HeaderProps) {
  const navigation = content?.navigation || defaultContent.navigation;
  const business = content?.business || defaultContent.business;
          ...
          <div className="flex items-center justify-between">
            ...
            <nav className="hidden md:flex items-center gap-10">
              {navigation.map((item, index) => (
                <a
                  ...
                  } ${index === 0 && !overDarkHero ? "text-brand" : ""}`}
                >
                  {item.name}
                </a>
              ))}
            ...
            <button
              ...
              className={`md:hidden p-2 transition-colors duration-300 ${
                overDarkHero && !isMobileMenuOpen ? "text-white" : "text-brand"
              }`}
              aria-label="Menu"
            >
              {isMobileMenuOpen ? (
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            className="fixed inset-0 z-40 bg-base-50 pt-28"
          >
            <motion.nav
