...
export default function PhilosophySection({ content }: PhilosophySectionProps) {
  const data = content || defaultContent.philosophy;
  const paragraphs = data.text.split('\n\n').filter(p => p.trim());

  return (
    <section className="relative section-padding pb-28 bg-accent-200">
      <div className="max-w-3xl mx-auto px-6 md:px-8">
        ...
        <motion.div
          ...
          className="bg-accent-100/50 rounded-2xl p-10 md:p-14 text-center relative"
        >
          {/* Large Quote Mark */}
          <div className="quote-mark absolute top-4 left-8 select-none">&ldquo;</div>

          {/* Quote Text */}
          <p className="font-serif text-xl md:text-2xl text-text-dark leading-relaxed mb-6 relative z-10">
            «{data.quote.text}»
          </p>
