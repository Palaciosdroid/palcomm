...
export default function CertificatesSection({ content }: CertificatesSectionProps) {
  const data = content || defaultContent.certificates;
  // Duplicate certificates for seamless infinite scroll
  const duplicatedCerts = [...data.certificates, ...data.certificates];

  return (
    <section className="py-16 md:py-24 gradient-accent overflow-hidden">
        ...
        >
          {duplicatedCerts.map((cert, index) => (
            ...
            >
              ...
              <div className="h-64 md:h-80 lg:h-96 bg-white rounded-2xl shadow-card border border-accent-100 overflow-hidden hover:shadow-medium transition-shadow duration-300">
